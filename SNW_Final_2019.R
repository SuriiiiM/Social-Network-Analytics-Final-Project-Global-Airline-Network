...
colnames(route) <- c('Airline','Source_Airport','Destination_Airport','Codeshare','Stops','Plane')
...
data <- merge(route[,c(1,2,3,4,5)],airport[,c(3,4,5,7,8,9,10)],by.x = "Source_Airport",by.y="IATA",all.x=TRUE )
...
uniqueAirline <- unique(data$Airline) #607
uniqueRoute <- unique(data[,c(1,2)]) #25266
uniqueAirportD <- unique(data$Destination_Airport) #3466
uniqueAirportS <- unique(data$Source_Airport) #1594
uniqueAirport <- unique(append(uniqueAirportD,uniqueAirportS)) #3555
uniqueCountry <- unique(append(data$Country_S,data$Country_D)) #230
uniqueCity <- unique(append(data$City_S,data$City_D)) #3283

#codeshare involved
nshare <- sum(data$Codeshare=="*") #2903

#data remove codeshare route
datac <- data[which(data$Codeshare!="*"),]
nrow(datac) #30800
...
InternationRoute <- datac[which(datac$Country_S!=datac$Country_D),] #14708
nrow(InternationRoute)/nrow(datac) #47.75%
...
a1 <- as.data.frame(authority_score(AirportG)$vector)
h1 <- as.data.frame(hub_score(AirportG)$vector)
...
#Airport Level
centr_clo(AirportG,normalized=TRUE)$centralization 
#undirected 0.008037011

centr_betw(AirportG,normalized=TRUE)$centralization 
#0.07211321

centr_degree(AirportG,normalized=TRUE)$centralization 
#0.08057694
...
clique_num(AirportG) #24
clique_num(CityG) #25
clique_num(CountryG) #22

clique_num(AirportG) #24
CliqueAirport <- largest_cliques(AirportG)
CliqueA <- as.data.table(matrix(data=names(unlist(CliqueAirport)),byrow = TRUE,ncol=24))
write_xlsx(CliqueA, path = "CliqueAirport.xlsx", col_names = TRUE)

clique_num(CityG) #25
CliqueCity <- largest_cliques(CityG)
CliqueC <-as.data.table(matrix(data=names(unlist(CliqueCity)),byrow = TRUE,ncol=25))
write_xlsx(CliqueC,path = "CliqueCity.xlsx", col_names = TRUE)

clique_num(CountryG) #22
CliqueCountry <- largest_cliques(CountryG)
CliqueCo <-as.data.table(matrix(data=names(unlist(CliqueCountry)),byrow = TRUE,ncol=22))
